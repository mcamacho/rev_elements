...
// FUNCTIONS
    ...
    newLocation = function () {
        var search = '', condx, reg = /condition|ext_color_generic|make|standard_body|standard_model|year/;
    ...
    getCount = function () {
        /*jQuery('.search-top-page .wrap_show').show();*/
    ...
    selectEmpty = function (msg, filter) {
        var nume, value, ulwidth, tMax, tMax1, nminor, nmayor, rate, msgn, ini = 0, tempList = [], calini, parentB, primarymake;
        if (msg !== '') {
            ...
            if (filter) {
                ...
                for (nume = 1; tMax > rate && nume < Math.floor(tMax1 / rate); nume += 1) {
                    if (filter === 'milage' && (nume * rate) >= 100000) { break; }
        ...
        /*if (msg.length > 12) {
            ulwidth = (Math.ceil(Math.sqrt(msg.length / 4)) * 104).toString(10);
        } else {
            ulwidth = '104';
        }*/
        /*jQuery('ul.opt[title="' + butSelected + '"]').css({'width': ulwidth, 'marginLeft': (ulwidth / 208 * -104) + (parentB / 2)});*/
    ...
    defaultImage = function () {
        jQuery('#vehicle-list')
            .on('error', 'img:not(.logos img)', function () {
                jQuery(this).attr({
                    src: '{src}http://cdn.hostrevo.com/assets/img/no-image.jpg&w=160&h=120&zc=2&q=70',
                    alt: 'Image Unavailable'
                });
            })
            .on('click', '.vehicle-pt-item-left a, .check-available a, .title-wrap a', function () {
                newLocation();
            });
      // hover for disclaimer
        if (disclaimer) {
            jQuery('#vehicle-list')
                .on('mouseenter', '.disclaimer', function () {
    ...
    addFilter = function (id, view) {
        ...
        if (view) { eleAjax(); }// when view is true update vehicle list
    };
    // compare the json object with the current location path array
    compareArray = function (jso, type) {
        var xx0, xx1, re = /[=]/, re1 = /fuel_type|milage|mpg_city|mpg_hwy|msrp|price|transmission/,
            ...
            allArrayObj = { make: makeArray, standard_model: modelArray, standard_body: bodyArray, ext_color_generic: colorArray, year: yearArray, vin: vinArray, stock: stockArray };
    ...
    addOptions = function () {
        ...
        jQuery.getJSON(
            '{THEME_ROOT}/_ajax.php?searchAI=true',
            function (msg) {
                if (pArray[0] === 'inventory') {
                    compareArray(msg);
                } else {
    ...
    acomplete = function (jso) {
        ...
        /*var jsoarray = [],
            ja;
        for (ja in jso) {
            if (jso.hasOwnProperty(ja)) {
                jsoarray = jsoarray.concat(jso[ja]);
            }
        }*/
        jsoarray = jsoarray.concat(jso.stock, jso.vin);
        /*compareArray(jso, jQuery(this).val()); jQuery(this).val('');*/
        jQuery('#vin').autocomplete({
            ...
            //.blur(function () { window.location = '{home}/vehicle/' + jQuery(this).val(); })
            //.keydown(function (event) { if (event.witch === 13) { window.location = '{home}/vehicle/' + jQuery(this).val(); } })
            //.keypress(function (event) { if (event.witch === 13) { window.location = '{home}/vehicle/' + jQuery(this).val(); } });
    };
    ...
    next = function () {
        ...
        jQuery.ajax({
            ...
            url: ajaxPHP,
            success: function (msg) {
                /*var vehicleCount = jQuery('#vehicle-list .vehicle').size();*/
                vehicleLoad(msg.view, 'next');
                scrollverify(msg.count);
                /*if (vehicleCount !== Number(msg.count)) {
                    scrollActive = true;
                }*/
            },
            complete: function () {
...
// On document ready
    jQuery(document).ready(function () {
        ...
        var msrpar = 'msrp';
        if (pArray[0] !== 'inventory') {
            //if ((pArray[0]) === 'model' || pArray.length === 1) {
                if (typeof (customDrill) !== 'undefined' && customDrill.force) {
                    ...
                    jQuery('#condition').removeAttr('disabled')
                        .val('New').change()
                        .next().css('visibility', 'visible');
                    msrpar = jQuery('button[value="msrp"]').attr('new');
                }
            //}
            jQuery('.bcrumb').css('width', '96%');
        ...
        jQuery('#filter button:not([value="sort"])').click(function () {
            ...
            if (!jQuery(this).hasClass('ready')) {
                ...
                if (nextul.children().length > 0) {
                    nextul.is(':visible') ? nextul.hide() : nextul.show();
                }
            }
        ...
        jQuery('ul.opt').on('click', 'a', function (e) {
            ...
            jQuery('input.newFilter').attr({ 'id': jQuery(prevBut).val(), 'value': selection });
            jQuery('ul.opt').empty();
        ...
        // Stock and Vin Search
        /*jQuery('#stock, #vin').change(function (event) {
            addFilter(event.target.id, true);
        });*/
        // sort functionality
        ...
        jQuery('.showresults').click(function (e) {
            e.preventDefault();
            var search = '', reg = /condition|ext_color_generic|make|standard_body|standard_model|year/;
            for (cond = 0; cond < conditions.length; cond += 1) {
                if (reg.test(conditionsId[cond])) {
